...
async def test__route_present_in_one_version__other_version_return_404(client: TestClient):
    result = client.get("/items", headers={"x-version": "2"})
    assert result.status_code == 200
    result = client.get("/items", headers={"x-version": "1"})
    assert result.status_code == 404
...
@pytest.mark.parametrize("path", ["/hello/", "/items/"])
async def test__any_route_redirect_slashes__should_perform_redirect(client, path: str):
    result = client.get(path, headers={"x-version": "2"})
    assert result.status_code == 200
